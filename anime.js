...
bot.registerCommand('ping', msg => {
  ...
  bot.registerCommand("p", (msg, args) => {
    ...
    if (args[0] == "steak"){
        msg.channel.createMessage('', {file, name: 'steak.jpg'});
    }
    else if (args[0] == "knife"){
        file = fs.readFileSync('./knife.jpg');
        msg.channel.createMessage('', {file, name: 'knife.jpg'});
    }
    else if (args[0] == "hack"){
...
tagCommand.registerSubcommand("create", (msg,args) => {
    ...
    connection.query("INSERT INTO tags (tag, value) VALUES (\'"+args[0] + "\', \'"+arr2.toString()+"\')"),
...
bot.registerCommand("r", (msg, args) =>{
    try {let result = droll.roll(args[0]);
    if (result.rolls[0] == 1){
...
},  
{
    ...
    usage: "Usage: \`uwu hug @xamtheking\`",
    argsRequired: true,
    guildOnly: true
});
*/
