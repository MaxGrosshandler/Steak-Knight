            ...
            if (typeof monster == "undefined"){
                ...
                if ( 0 >= monster.hp - playerHit){
                    ...
                    if (player.player_xp + monster.monster_level*20 >= player.player_next_level){
                        client.query("UPDATE players SET (xp, hp, atk, level, next_level, maxhp) = (players.xp + $1, players.maxhp + 10 , players.atk + 1, players.level+1, players.next_level + 100, players.maxhp+10)   where id = $2", [monster.monster_level * 20, player.id]);
